...
static id<MTLRenderPipelineState> mtlfons__renderPipelineState(MTLFONScontext* mtl) {
...
static void mtlfons__renderDraw(void* userPtr, const float* verts, const float* tcoords, const unsigned int* colors, int nverts)
{
    ...
    MTLFONSvertex *vertexData = (MTLFONSvertex *) mtl->vertexBuffer.contents + mtl->currentVertexBufferOffset;
...
FONScontext* mtlfonsCreate(id<MTLDevice> device, int width, int height, int flags)
{
